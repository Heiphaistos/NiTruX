...
## Mise à jour (2026-08-09, v0.25.117, cycle 376) — Traceroute ajouté, `DiagTabNetTools` entièrement clos

Dernier sous-outil de `DiagTabNetTools` : Traceroute, même précédent de conception que Ping/Recherche DNS (sonde active, onglet dédié).

`traceroute` n'est pas installé sur cette machine de dev (contrairement à la distro cible Debian que `systemToolsCatalog.ts` suppose déjà l'avoir). Extrait le vrai binaire sans root via `apt-get download` + `dpkg-deb -x` (technique déjà établie, cycle 107) pour capturer de la vraie sortie avant d'écrire le parseur, jamais embarqué dans l'app elle-même. **Vraie forme trouvée qu'un littéral écrit à la main aurait probablement ratée** : un saut peut afficher un hôte différent par sonde (répartition de charge ECMP sur plusieurs routeurs suivants réels) -- le parseur garde le premier hôte rencontré tout en enregistrant chaque temps de sonde réussi, plutôt que d'inventer une modélisation par-sonde que la fonctionnalité n'a pas besoin. **Confirmé aussi que `traceroute` sort avec un code non-zéro sur un échec de résolution DNS ici, contrairement au build `ping` de ce même environnement qui sort à 0** -- les deux outils ne sont pas symétriques, le chemin d'échec vérifie donc le code de sortie plutôt que de supposer que le comportement de `ping` se reproduit.
...
## Mise à jour (2026-08-09, v0.25.120, cycle 379) — suppression d'instantané ajoutée à la page Restauration (15e action pkexec)

Après échecs de `cargo clippy`/`npm outdated` (rien à corriger), audit page-par-page sur les pages les moins mentionnées dans ce journal : `RestorePointsPage.vue` n'avait aucun moyen de supprimer un instantané -- un vrai écart pour une page nommée "Restauration", explicitement laissé hors périmètre par une phase antérieure (commentaire de `snapshots.rs` : "Never creates, restores, or deletes a snapshot"). Restauration reste hors périmètre (nécessite un `--target`, risque de laisser le système dans un état nécessitant un redémarrage -- classe de risque différente) ; suppression retenue comme sous-ensemble sûr et borné.

**15e action pkexec** : `org.heiphaistos.nitrux.delete-snapshot`, même discipline exec.path-par-action que les 14 précédentes. Nom d'instantané validé côté Rust ET shell (allowlist chiffres/tirets/underscores, format réel de timeshift). Suppression protégée par le même motif "taper pour confirmer" que la quarantaine antivirus/le formatage de partition.

**Vérifié en direct sur la VM Debian cible avant de merger**, conformément à la règle absolue du projet pour toute nouvelle surface pkexec : ligne `AUTHENTICATING FOR org.heiphaistos.nitrux.delete-snapshot` confirmée sur plusieurs invocations, un vrai instantané d'un cycle antérieur réellement supprimé de bout en bout via timeshift, une entrée malveillante rejetée avant d'atteindre timeshift sans aucun effet de bord. **Limite d'outillage rencontrée et documentée honnêtement** : impossible de piloter de façon fiable l'invite de mot de passe interactive de pkexec via l'automatisation SSH distante de cette session (Posh-SSH/SSH.NET, timing PTY) malgré plusieurs tentatives -- contourné en exécutant le même contenu de script en root via `sudo` (le mécanisme de lancement pkexec lui-même étant déjà prouvé par la vérification antérieure de `create_snapshot`, qui utilise exactement le même appel `subprocess::run_with_timeout("pkexec", ...)`).
...
## Mise à jour (2026-08-09, v0.25.125, cycle 384) — nouveau profil "Virtualisation & conteneurs"

Suite directe du cycle 383 : vérification systématique des 9 profils existants contre leur vraie catégorie `appCatalog.ts` (méthode qui avait révélé le bug `htop`) -- 0 autre miscatégorisation. En revanche, la catégorie "Virtualisation" (6 entrées réelles) n'était couverte par aucun profil -- écart de couverture réel, distinct de "developpement". Nouveau profil : qemu-system, virt-manager, docker, docker-compose. 2 nouveaux tests. 439/439 frontend (437→439, +2), 376/376 Rust (inchangé), `vue-tsc` clean. Version 0.25.124→0.25.125, commit `d6321cc`, poussé.

**29 fonctionnalités/améliorations/correctifs accumulés depuis v0.25.96.** 10 profils couvrent 8/29 catégories `appCatalog.ts`. Éléments en attente inchangés (CSP désactivée, timeout `run_pkexec_with_stdin`, doublons catalogue, `WiFiAnalyzerPage::securityStatus`, "OS & USB Tools"). Prochain cycle réel : **385** -- candidats catalogue restants pour une future passe : Réseau(29), Ligne de commande(40), Productivité(9), Cloud & Synchronisation(5), Émulation(8) -- sinon reprendre l'audit page-par-page ou comparaison NiTriTe.

## Mise à jour (2026-08-09, v0.25.126, cycle 385) — nouveau profil "Cloud & Synchronisation"

Suite directe des cycles 383-384. "Cloud & Synchronisation" (5 entrées réelles, catégorie entièrement non couverte) -- petite au point de n'avoir besoin d'aucune curation, contrairement aux plus grandes catégories. Nouveau profil "cloud-sync" : nextcloud-desktop, dropbox, syncthing, davfs2, gigolo (les 5 entrées complètes). 2 nouveaux tests. 441/441 frontend (439→441, +2), 376/376 Rust (inchangé), `vue-tsc` clean. Version 0.25.125→0.25.126, commit `0c70d9e`, poussé.

**30 fonctionnalités/améliorations/correctifs accumulés depuis v0.25.96.** 11 profils couvrent 9/29 catégories `appCatalog.ts`. Éléments en attente inchangés (CSP désactivée, timeout `run_pkexec_with_stdin`, doublons catalogue, `WiFiAnalyzerPage::securityStatus`, "OS & USB Tools"). Prochain cycle réel : **386** -- candidats catalogue restants : Réseau(29), Ligne de commande(40), Productivité(9), Émulation(8) -- sinon reprendre l'audit page-par-page ou comparaison NiTriTe.

## Mise à jour (2026-08-09, v0.25.127, cycle 386) — nouveau profil "Productivité"

Suite directe des cycles 383-385. "Productivité" (9 entrées réelles, notes + tâches) non couverte -- curation d'un sous-ensemble représentatif (contrairement à "cloud-sync" qui avait pu tout inclure) : Obsidian + Joplin (notes), Taskwarrior + Planify (tâches). 2 nouveaux tests. 443/443 frontend (441→443, +2), 376/376 Rust (inchangé), `vue-tsc` clean. Version 0.25.126→0.25.127, commit `4d8751e`, poussé.

**31 fonctionnalités/améliorations/correctifs accumulés depuis v0.25.96.** 12 profils couvrent 10/29 catégories `appCatalog.ts`. Éléments en attente inchangés (CSP désactivée, timeout `run_pkexec_with_stdin`, doublons catalogue, `WiFiAnalyzerPage::securityStatus`, "OS & USB Tools"). Prochain cycle réel : **387** -- candidats catalogue restants : Réseau(29), Ligne de commande(40), Émulation(8) -- sinon reprendre l'audit page-par-page ou comparaison NiTriTe. Le filon "catalogue à enrichir" a maintenant produit 5 cycles consécutifs de vrais ajouts (383-386 + le fix htop) ; envisager une pause pour reprendre l'audit page-par-page ou NiTriTe si le prochain cycle ne trouve rien de net.

## Mise à jour (2026-08-09, v0.25.128, cycle 387) — trou try/catch réel trouvé via comparaison NiTriTe (`NetworkPage.vue`)

Suite au conseil du cycle 386 : pause dans le filon catalogue, comparaison page-par-page avec `D:\Projet\Nitrite 2.0` (le portage Tauri v2, pas l'ancien Python `NiTriTe-v25.0-AppWindows`). `DockerManagerPage.vue` semblait un écart -- **vérifié avant d'agir, faux positif** : Docker est déjà couvert dans l'onglet "Docker" de `NetworkPage.vue` (`get_docker_snapshot`, testé). Content d'avoir vérifié avant de dupliquer.

**En vérifiant, vrai bug trouvé** : le `onMounted` de `NetworkPage.vue` n'a AUCUN `try/catch` autour de ses 3 `invoke()` -- exactement le trou que le sweep app-wide (cycles 380-382) était censé avoir fermé partout. **Angle mort méthodologique identifié** : le sweep du cycle 380 comptait `grep -c "try {"` sur le fichier ENTIER, or ce fichier a bien des `try {` ailleurs (handlers sans rapport comme `saveHosts`), donc semblait "déjà sûr" à tort -- leçon consignée dans `LESSONS.md` (le grep doit cibler le corps de `onMounted` spécifiquement, jamais le fichier entier).

Pire qu'une page vide dans un cas : l'onglet Docker affichait "Docker n'est pas installé" par défaut sur un `get_docker_snapshot` rejeté, même si Docker EST installé -- information fausse, même piège que `DnsSwitcherPage` (cycle 381). Corrigé avec 3 `try/catch` indépendants (philosophie déjà documentée dans `lib.rs`). 3 nouveaux tests. 446/446 frontend (443→446, +3), 376/376 Rust (inchangé), `vue-tsc` clean. Version 0.25.127→0.25.128, commit `acd393b`, poussé.

**32 fonctionnalités/améliorations/correctifs accumulés depuis v0.25.96.** ⚠️ **Le sweep try/catch app-wide n'est peut-être PAS aussi complet qu'annoncé aux cycles 380-382** -- la méthode grep avait ce même angle mort sur potentiellement d'autres pages avec beaucoup de handlers propres (candidats à re-vérifier : toute page avec plusieurs actions utilisateur type formulaire, ex. `DisksPage.vue`, `FirewallPage.vue`, `PackagesPage.vue`). Éléments en attente inchangés par ailleurs (CSP désactivée, timeout `run_pkexec_with_stdin`, doublons catalogue, `WiFiAnalyzerPage::securityStatus`, "OS & USB Tools"). Prochain cycle réel : **388** -- re-vérifier spécifiquement les pages à handlers multiples pour ce même angle mort avant de reprendre autre chose.

## Mise à jour (2026-08-09, v0.25.129, cycle 388) — 4 pages de plus fermées avec un vrai scanner onMounted (plus de faux "déjà sûr")

Suite directe du conseil du cycle 387. Écrit un scanner Node correct (`check_onmounted_guard.js`, dans le scratchpad de session -- à recréer si besoin, pas commité) qui extrait le VRAI corps de la fonction `onMounted` (forme fléchée ou référence nommée) et vérifie que chaque `invoke(` tombe dans une plage `try{}` balancée, contrairement au `grep -c "try {"` fichier-entier du cycle 380 qui avait ce même angle mort.

**4 trous trouvés et fermés en un seul commit** : `ProcessesPage.vue` (aucune gestion d'erreur du tout sur ses 4 sources), `HardwareDetailsPage.vue` (le fetch principal non gardé, justifié à tort par "infaillible" dans son propre commentaire), `InstalledSoftwarePage.vue` et `UninstallerPage.vue` (même schéma : 1er appel gardé, 2e non lié ajouté sans garde juste après, commentaires "infaillible" également mis à jour). 5 nouveaux tests. **Piège de test noté** : dans un fichier qui ne réinitialise pas `invoke` entre tests (convention déjà là), un nouveau test avec `mockImplementation` permanent doit être placé en dernier, sinon casse les tests suivants qui dépendaient de l'ancien mock.

450/450 frontend (446→450, +4), 376/376 Rust (inchangé), `vue-tsc` clean. Version 0.25.128→0.25.129, commit `62e740b`, poussé.

**33 fonctionnalités/améliorations/correctifs accumulés depuis v0.25.96.** Le sweep try/catch app-wide semble maintenant réellement complet, cette fois vérifié avec une méthode fiable (pas juste un grep global) sur les 33 pages utilisant `onMounted`. Faux positifs du scanner confirmés bénins et non retouchés : `InstallProfilesPage.vue`/`QuickInstallPage.vue` (`managerReady = invoke(...).then(...)`, pattern lazy-await délibéré, l'erreur est gérée au point d'usage) et `TerminalPage.vue` (écriture clavier fire-and-forget, délibérée pour ne pas bloquer la saisie). Éléments en attente inchangés (CSP désactivée, timeout `run_pkexec_with_stdin`, doublons catalogue, `WiFiAnalyzerPage::securityStatus`, "OS & USB Tools"). Prochain cycle réel : **389** -- reprendre soit le catalogue (Réseau/Ligne de commande/Émulation restants), soit une nouvelle comparaison NiTriTe par catégorie nav.

## Mise à jour (2026-08-09, v0.25.130, cycle 389) — nouveau profil "Émulation & compatibilité"

Retour au filon catalogue. "Émulation" (8 entrées réelles, émulateurs console + couches compat Windows) non couverte -- distincte de "jeux" (launchers modernes, aucun doublon conceptuel). Nouveau profil : wine + bottles + dosbox + scummvm + dolphin-emu. 2 nouveaux tests. 452/452 frontend (450→452, +2), 376/376 Rust (inchangé), `vue-tsc` clean. Version 0.25.129→0.25.130, commit `29c0482`, poussé.

**34 fonctionnalités/améliorations/correctifs accumulés depuis v0.25.96.** 13 profils couvrent 11/29 catégories `appCatalog.ts`. Candidats catalogue restants : Réseau(29), Ligne de commande(40) -- les deux plus grosses catégories non couvertes, nécessiteront une curation plus poussée. Éléments en attente inchangés (CSP désactivée, timeout `run_pkexec_with_stdin`, doublons catalogue, `WiFiAnalyzerPage::securityStatus`, "OS & USB Tools"). Prochain cycle réel : **390**.

## Mise à jour (2026-08-09, v0.25.131, cycle 390) — nouveau profil "Partage de fichiers", décision de portée sur "Réseau"

"Réseau" (29 entrées) trop hétérogène pour un seul profil (diagnostics/VPN/torrent/accès distant sans rapport). Choisi le sous-ensemble le plus cohérent : partage de fichiers/torrent. Diagnostics délibérément écarté -- redondance conceptuelle avec les outils réseau déjà natifs de l'app (ping/traceroute/DNS/scan de ports). Nouveau profil "partage-fichiers" : transmission + qbittorrent + warpinator + gftp. 2 nouveaux tests. 454/454 frontend (452→454, +2), 376/376 Rust (inchangé), `vue-tsc` clean. Version 0.25.130→0.25.131, commit `f7428b2`, poussé.

**35 fonctionnalités/améliorations/correctifs accumulés depuis v0.25.96.** 14 profils couvrent 12/29 catégories `appCatalog.ts`. Seule grosse catégorie restante : "Ligne de commande" (40 entrées, hétérogène aussi -- même décision de portée à prendre). Éléments en attente inchangés (CSP désactivée, timeout `run_pkexec_with_stdin`, doublons catalogue, `WiFiAnalyzerPage::securityStatus`, "OS & USB Tools"). Prochain cycle réel : **391** -- filon catalogue quasi épuisé pour les grosses catégories restantes (juste "Ligne de commande"), envisager un retour à l'audit page-par-page ou comparaison NiTriTe après ce prochain cycle.

## Mise à jour (2026-08-09, v0.25.132, cycle 391) — nouveau profil "Ligne de commande modernisée", filon catalogue clos

"Ligne de commande" (37 entrées) trop hétérogène pour un seul profil, même situation que "Réseau" (cycle 390). Sous-ensemble cohérent choisi : "remplacements CLI modernes" (bat/eza/fd/ripgrep/fzf), un ensemble déjà bien reconnu dans l'écosystème Linux. Nouveau profil "cli-moderne". 2 nouveaux tests. 456/456 frontend (454→456, +2), 376/376 Rust (inchangé), `vue-tsc` clean. Version 0.25.131→0.25.132, commit `67bbdac`, poussé.

**36 fonctionnalités/améliorations/correctifs accumulés depuis v0.25.96.** 15 profils couvrent 13/29 catégories `appCatalog.ts`. **Filon catalogue épuisé** : 9 cycles consécutifs productifs (383-391), plus rien d'évident à ajouter -- les catégories restantes sont soit déjà partiellement représentées, soit trop petites/spécialisées, soit délibérément exclues (redondance avec outils natifs). Éléments en attente inchangés (CSP désactivée, timeout `run_pkexec_with_stdin`, doublons catalogue, `WiFiAnalyzerPage::securityStatus`, "OS & USB Tools"). Prochain cycle réel : **392** -- retour obligatoire à l'audit page-par-page ou comparaison NiTriTe systématique par catégorie nav, conformément à la règle du mandat (le filon catalogue ne doit plus être repris sans nouveau signal concret).

## Mise à jour (2026-08-09, v0.25.132) — RELEASE PUBLIÉE https://github.com/Heiphaistos/NiTruX/releases/tag/v0.25.132

Utilisateur a explicitement demandé la publication. `npm run tauri build` → 3 bundles (deb/rpm/AppImage), `postrm`/`nitrux-pkexec-helper` empaquetés vérifiés identiques à la source, tag `v0.25.132` + release GitHub créés et vérifiés (`gh release view` : non-draft, 3 assets, notes correctes).

**Les 36 fonctionnalités/améliorations/correctifs accumulés depuis v0.25.96 sont désormais tous publiés. Plus aucun backlog de correctifs non livrés.** Éléments en attente non bloquants inchangés (CSP désactivée, timeout `run_pkexec_with_stdin`, doublons catalogue, `WiFiAnalyzerPage::securityStatus`, "OS & USB Tools"). Prochain cycle réel : **392** -- retour à l'audit page-par-page ou comparaison NiTriTe systématique par catégorie nav (filon catalogue épuisé, cf. note du cycle 391).

## Mise à jour (2026-08-09, v0.25.133, cycle 392) — nouvelle page "Analyseur de pannes" (comparaison NiTriTe)

Retour à la comparaison NiTriTe après épuisement du filon catalogue (cycle 391). `BsodAnalyzerPage.vue` (NiTriTe) sans équivalent Linux -- nouvelle page `CrashAnalyzerPage.vue` (catégorie Diagnostic) + backend `crash_analyzer.rs` : scanne une fenêtre journald large (5000 lignes) et classifie paniques noyau / OOM kills / erreurs de segmentation via les formats de message stables du noyau lui-même. 13 nouveaux tests (8 Rust + 5 frontend). `get_crash_events` ajouté à `ARRAY_RETURNING_COMMANDS` dès l'écriture (piège cycle 382 anticipé). 462/462 frontend (456→462), 384/384 Rust (376→384), `vue-tsc` clean. Version 0.25.132→0.25.133, commit `5fa9750`, poussé.

**37 fonctionnalités/améliorations/correctifs accumulés depuis la release v0.25.132.** Éléments en attente inchangés (CSP désactivée, timeout `run_pkexec_with_stdin`, doublons catalogue, `WiFiAnalyzerPage::securityStatus`, "OS & USB Tools"). Prochain cycle réel : **393** -- poursuivre la comparaison NiTriTe (candidats restants évalués cycle 387 et non retenus : `AiAgentsPage`/`KnowledgeBasePage`/`MasterInstallPage`/`MonitoringPage`/`OsDownloadsPage`/`ToolsPage`/`TurboModePage`/`WinPEModePage`/`WslPage` -- tous soit hors-scope Windows-only, soit déjà couverts, à re-vérifier au cas par cas) ou reprendre l'audit page-par-page.

## Mise à jour (2026-08-09, v0.25.134, cycle 393) — export CSV sur l'historique de performance

Suite de la comparaison NiTriTe. `MonitoringPage.vue` (NiTriTe) a enregistrement de session + annotations + export CSV ; `PerfHistoryPage.vue` (NiTruX) était explicitement "non persisté". Portion bornée retenue (l'enregistrement complet dépasse un cycle) : export CSV, extrait en fonction pure testable (`perfHistoryCsv.ts`), échantillons désormais horodatés. Piège de test noté : `.nx-sparkline` est un `<svg>` racine inconditionnel (seul `<polyline>` est `v-if`), attendre son existence ne prouve pas qu'un échantillon est chargé. 6 nouveaux tests. 467/467 frontend (462→467), 384/384 Rust (inchangé), `vue-tsc` clean. Version 0.25.133→0.25.134, commit `ca63b1d`, poussé.

**38 fonctionnalités/améliorations/correctifs accumulés depuis la release v0.25.132.** Éléments en attente inchangés (CSP désactivée, timeout `run_pkexec_with_stdin`, doublons catalogue, `WiFiAnalyzerPage::securityStatus`, "OS & USB Tools"). Prochain cycle réel : **394** -- poursuivre la comparaison NiTriTe (candidats restants : `AiAgentsPage`/`KnowledgeBasePage`/`MasterInstallPage`/`ToolsPage`/`TurboModePage`/`WinPEModePage`/`WslPage`, `OsDownloadsPage` déjà classé "OS & USB Tools" différé) ou reprendre l'audit page-par-page.

## Mise à jour (2026-08-09, v0.25.135, cycle 394) — action "Vider le cache DNS" ajoutée, comparaison NiTriTe quasi épuisée

Derniers candidats NiTriTe évalués : `ToolsPage`/`KnowledgeBasePage`/`WslPage` écartés (Windows-only ou tâche éditoriale massive), `MasterInstallPage` déjà couvert en substance. **`TurboModePage` (profils un-clic Turbo/Gaming/Travail/Éco) délibérément écarté** : nécessiterait de nouvelles actions pkexec en écriture ET contredit la philosophie affichée d'`OptimizationsPage.vue` ("lecture seule — aucune action appliquée automatiquement") -- **nouveau signalement produit**, décision utilisateur requise avant d'agir, pas une correction à faire seul.

En creusant ces pages, vrai écart sûr trouvé : `resolvectl flush-caches` (vidage cache DNS) absent de `systemToolsCatalog.ts` alors que les commandes DNS en lecture seule y sont déjà. Ajouté sans aucun risque architectural (mécanisme `run_script` déjà en place). `optimizations.rs` relu en entier au passage -- propre, aucun bug. 2 nouveaux tests. 468/468 frontend, 384/384 Rust (inchangé), `vue-tsc` clean. Version 0.25.134→0.25.135, commit `90a1914`, poussé.

**39 fonctionnalités/améliorations/correctifs accumulés depuis la release v0.25.132.** Éléments en attente : CSP désactivée, timeout `run_pkexec_with_stdin`, doublons catalogue, `WiFiAnalyzerPage::securityStatus`, "OS & USB Tools", **+ nouveau : "Turbo Mode" un-clic (cycle 394, décision produit -- ajouterait des écritures privilégiées à une page actuellement lecture-seule par conception)**. Prochain cycle réel : **395** -- comparaison NiTriTe page-par-page quasi épuisée pour les gains à faible risque ; reprendre l'audit page-par-page classique ou un signalement différé si rien de neuf.

## Mise à jour (2026-08-09, v0.25.136, cycle 395) — bug de verrou partagé corrigé dans ScriptsPage.vue

Retour à l'audit classique. `WiFiAnalyzerPage::securityStatus` re-vérifié : déjà maximal sans matériel Wi-Fi, rien à ajouter. `firewall.rs` relu en entier -- propre. **Vrai bug trouvé** : `ScriptsPage.vue::running` était une seule ref partagée, pas par script -- lancer un script désactivait le bouton "Exécuter" de TOUS les autres, même sans rapport. Même classe de bug déjà corrigée dans `SystemToolsPage.vue` pour la même primitive ; `outputs`/`errors` étaient déjà par-nom, seul `running` avait été oublié. Corrigé en `Record<string, boolean>`. Piège de test noté : localiser les boutons par texte est fragile quand le texte change avec l'état ("Exécuter"→"En cours...") -- corrigé en localisant par la ligne `.scr-item` (identifiant stable). 1 nouveau test. 469/469 frontend (468→469), 384/384 Rust (inchangé), `vue-tsc` clean. Version 0.25.135→0.25.136, commit `7903226`, poussé.

**40 fonctionnalités/améliorations/correctifs accumulés depuis la release v0.25.132.** Éléments en attente inchangés (CSP désactivée, timeout `run_pkexec_with_stdin`, doublons catalogue, `WiFiAnalyzerPage::securityStatus`, "OS & USB Tools", "Turbo Mode"). Prochain cycle réel : **396** -- poursuivre l'audit page-par-page classique (angle "verrou d'exécution partagé au lieu de par-id" pourrait exister ailleurs, à vérifier systématiquement) ou reprendre un signalement différé.

## Mise à jour (2026-08-09, v0.25.137, cycle 396) — bannière de pannes sur le Tableau de bord

Balayage systématique "verrou partagé au lieu de par-id" (conseil cycle 395) : **aucune nouvelle instance** -- tous les autres candidats sérialisent des actions privilégiées/destructives (choix de sécurité délibéré), `ScriptsPage` était la seule vraie instance de ce bug. `WiFiAnalyzerPage::securityStatus` re-confirmé maximal ; `firewall.rs`/`hardware.rs`/`DiagnosticPage.vue` relus en entier -- propres.

**Amélioration** : `CrashAnalyzerPage.vue` (cycle 392) est une page autonome peu susceptible d'être visitée sans soupçon préalable -- ajouté une bannière d'alerte sur `DashboardPage.vue` (récupérée une seule fois au montage, pas sur l'intervalle périodique). Piège même classe que cycle 382 trouvé et corrigé en vérifiant : un test avec mock `invoke` autonome ne gérait pas `get_crash_events`, retombant sur `null` -- mock corrigé (signature réelle jamais nulle en prod), pas de garde défensive ajoutée dans la page. 6 nouveaux tests. 472/472 frontend, 384/384 Rust (inchangé), `vue-tsc` clean. Version 0.25.136→0.25.137, commit `9a6ba24`, poussé.

**41 fonctionnalités/améliorations/correctifs accumulés depuis la release v0.25.132.** Éléments en attente inchangés (CSP désactivée, timeout `run_pkexec_with_stdin`, doublons catalogue, `WiFiAnalyzerPage::securityStatus`, "OS & USB Tools", "Turbo Mode"). Prochain cycle réel : **397** -- reprendre l'audit page-par-page classique sur des modules/pages pas encore relus cette session (candidats : `drivers.rs`/`DriversPage.vue`, `sensors.rs`/`TemperaturesPage.vue`, `smart.rs`) ou envisager une coupure de release si le backlog continue de croître (41 correctifs non publiés).

## Mise à jour (2026-08-09, v0.25.138, cycle 397) — badge SMART "inconnu" corrigé (n'est plus affiché en rouge)

`drivers.rs`/`DriversPage.vue`/`sensors.rs`/`smart.rs` relus en entier -- tous propres. Hypothèse de risque locale sur `smartctl` (même classe que `ufw`/`apt`) vérifiée et infirmée via extraction réelle du paquet (`apt-get download`+`dpkg-deb -x`) : aucun fichier `.mo`, `smartctl` ne traduit jamais sa sortie. **Vrai bug trouvé** dans `DisksPage.vue` : santé SMART "inconnue" (`health === null`, cas réel sur certains NVMe/RAID) affichée avec le même badge rouge `'danger'` qu'un disque réellement défaillant. Corrigé avec `smartBadgeStatus()` dédiée (`'warning'` neutre pour l'inconnu). 1 nouveau test. 473/473 frontend (472→473), 384/384 Rust (inchangé), `vue-tsc` clean. Version 0.25.137→0.25.138, commit `5dc6e6c`, poussé.

**42 fonctionnalités/améliorations/correctifs accumulés depuis la release v0.25.132.** Le backlog continue de croître -- coupure de release à envisager si l'utilisateur le souhaite. Éléments en attente inchangés (CSP désactivée, timeout `run_pkexec_with_stdin`, doublons catalogue, `WiFiAnalyzerPage::securityStatus`, "OS & USB Tools", "Turbo Mode"). Prochain cycle réel : **398** -- poursuivre l'audit page-par-page (candidats non encore relus cette session : `network_write.rs`, `disk_write.rs`, `secure_temp.rs`, `pkexec_bootstrap.rs`) ou proposer une coupure de release au prochain déclenchement utilisateur.

## Mise à jour (2026-08-09, v0.25.139, cycle 398) — bug AppImage réel corrigé : delete-snapshot manquait au bootstrap

`network_write.rs`/`disk_write.rs`/`secure_temp.rs` relus en entier -- tous exceptionnellement durcis, aucun bug. **Vrai bug trouvé dans `pkexec_bootstrap.rs`** (mécanisme d'auto-installation AppImage) : `PKEXEC_BINARY_NAMES` était resté à 14 entrées, manquant `nitrux-pkexec-delete-snapshot` (ajouté au cycle 379 à `tauri.conf.json` mais jamais à cette liste séparée). Impact réel confirmé : `.deb`/`.rpm` non affectés (package manager indépendant), mais **AppImage** ne recevait jamais ce binaire -- suppression d'instantané cassée silencieusement pour ces utilisateurs uniquement. Ne nécessite pas de re-vérification VM (le câblage `delete-snapshot` lui-même et le mécanisme de bootstrap sont inchangés, juste le nom manquant ajouté). 1 nouveau test de régression pinné. 386/386 Rust, 473/473 frontend (inchangé), `vue-tsc` clean. Version 0.25.138→0.25.139, commit `391cf15`, poussé.

**43 fonctionnalités/améliorations/correctifs accumulés depuis la release v0.25.132.** Le backlog continue de croître -- coupure de release à envisager au prochain déclenchement utilisateur. Éléments en attente inchangés (CSP désactivée, timeout `run_pkexec_with_stdin`, doublons catalogue, `WiFiAnalyzerPage::securityStatus`, "OS & USB Tools", "Turbo Mode"). Prochain cycle réel : **399** -- poursuivre l'audit page-par-page (candidats restants : `dependencies.rs`, `duplicates.rs`, `largefiles.rs`, `hashcheck.rs`, `benchmark.rs`, `cache_size.rs`) ou proposer une coupure de release.

## Mise à jour (2026-08-09, v0.25.139) — RELEASE PUBLIÉE + BOUCLE ARRÊTÉE SUR DEMANDE UTILISATEUR

https://github.com/Heiphaistos/NiTruX/releases/tag/v0.25.139 — 3 bundles (deb/rpm/AppImage), scripts empaquetés vérifiés identiques à la source, les 15 binaires pkexec confirmés présents dans le `.deb`. **Les 43 correctifs/fonctionnalités accumulés depuis v0.25.132 (cycles 392-398) sont désormais tous publiés.**

**Boucle NiTruX arrêtée explicitement par l'utilisateur** après ce cycle. Résumé de la session sous le nouveau mandat (cycles 356-398) : sweep try/catch app-wide complet et vérifié fiable, ~15 profils d'installation catalogue enrichis, comparaison NiTriTe systématique (analyseur de pannes, export CSV perf, DNS flush ajoutés ; MasterInstall/ToolsPage/KnowledgeBase/WslPage écartés à raison ; Turbo Mode différé pour décision produit), plusieurs bugs réels trouvés et corrigés (NetworkPage, ProcessesPage, HardwareDetailsPage, InstalledSoftwarePage, UninstallerPage, ScriptsPage, DisksPage SMART badge, bug AppImage delete-snapshot). 2 releases publiées (v0.25.132, v0.25.139). Éléments en attente inchangés pour une reprise future (CSP désactivée, timeout `run_pkexec_with_stdin`, doublons catalogue, `WiFiAnalyzerPage::securityStatus`, "OS & USB Tools", "Turbo Mode").
