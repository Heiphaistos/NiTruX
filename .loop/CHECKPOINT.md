...
**Suspect restant en attente de VM** : exit-code `apt-autoremove` masqué côté script pkexec partagé (mentionné cycles 34/44, jamais réellement creusé depuis -- à reprendre si la VM reste joignable un prochain cycle).

**Nouveau piège d'infra découvert ce cycle** (à documenter dans LESSONS.md) : appeler un script Python avec un chemin `/tmp/...` en argument depuis Git Bash le fait réécrire en chemin Windows par la conversion MSYS avant que Python ne le voie -- `MSYS_NO_PATHCONV=1` en préfixe de la commande contourne ça.
