...
The disposable Debian 13 (trixie) test VM — already proven functional tonight for Phase 2 Part 2 and Phase 4 Part 2 — was reused for this plan's live pass. The rebuilt `.deb` (v0.6.0, incorporating all of Tasks 1–5) was installed via `apt-get install -y`, upgrading in place over the already-installed Phase 2/4 Part 2 baseline. `pkaction --verbose` confirmed all 3 new actions register with their own distinct, dedicated exec paths from the start (`nitrux-pkexec-troubleshoot`, `nitrux-pkexec-create-snapshot`, `nitrux-pkexec-quarantine-file`) — no shared-path ambiguity bug this time, since Task 1 designed around that lesson from the outset rather than retrofitting it.

### What was proven for real

- **`clean-cache`:** `pkexec /usr/bin/nitrux-pkexec-troubleshoot troubleshoot clean-cache` really ran `apt-get clean` — confirmed independently by counting `.deb` files in `/var/cache/apt/archives/` before (14) and after (1) the call.
- **`fix-broken`:** ran `dpkg --configure -a` + `apt-get install -f -y` for real; completed cleanly on a system with no actually-broken packages (0 upgraded/installed/removed), confirming the repair path executes without error on the common case.
- **`restart-network`:** `systemctl restart NetworkManager` really ran — the SSH session survived the restart (network self-healed as expected for this class of fix).
- **`vacuum-logs`:** `journalctl --vacuum-time=7d` really ran against all 3 real journal locations (`/run/log/journal`, `/var/log/journal`, and the machine-specific journal directory).
- **`create-snapshot`:** Timeshift was not pre-installed on the VM, so it was installed first (`apt-get install -y timeshift`) to test this for real rather than skip it. `pkexec /usr/bin/nitrux-pkexec-create-snapshot create-snapshot` triggered Timeshift's first-run auto-configuration (RSYNC mode, `/dev/sda2`) and genuinely created a real snapshot in 51 seconds, confirmed by Timeshift's own success message (`RSYNC Snapshot saved successfully (51s)`, tagged `ondemand`).
- **`quarantine-file`, success path:** a real disposable test file (`/tmp/nitrux-quarantine-test.txt`) was created and quarantined via `pkexec /usr/bin/nitrux-pkexec-quarantine-file quarantine-file /tmp/nitrux-quarantine-test.txt`. Verified independently afterward: the original path no longer exists, and the quarantined copy exists at `/var/lib/nitrux/quarantine/<timestamp>-nitrux-quarantine-test.txt` with permissions `----------` (`chmod 000`, confirmed via `ls -la` as root) inside a `drwx------` root-only directory — exactly matching the design and the documented `chmod-000-vs-root` limitation above (this blocks the original owner and any non-root actor; it does not claim to block root itself).
- **`quarantine-file`, rejection path:** `pkexec /usr/bin/nitrux-pkexec-quarantine-file quarantine-file '../etc/passwd'` was correctly rejected by the helper's own `validate_quarantine_path` (`quarantine path must be absolute`) *after* polkit authentication succeeded but *before* any `mv` — the real `/etc/passwd` was left untouched, confirming the defense-in-depth design (validation inside the privileged script itself, not just the Rust layer) works exactly as intended, same pattern already proven for `install-package`'s rejection path in Phase 2 Part 2.
- Every one of the 6 live invocations above showed the correct, matching `org.heiphaistos.nitrux.*` action id and consent message in the polkit auth prompt — confirming Task 1's from-the-start dedicated-exec-path design avoided the ambiguity bug that had to be fixed after the fact in the two prior plans.

### Updated release status

All items in the "NOT yet proven" section above are now resolved: the full `pkexec` chain has run for real for all 3 new actions (4 troubleshoot sub-actions + create-snapshot + quarantine-file, both success and rejection paths), the polkit dialog has been triggered and observed (via `pkttyagent`'s text-mode equivalent) showing the correct action every time, and `pkaction --verbose` confirms all 3 actions register correctly under their own dedicated exec paths.

**Still not exercised:** the GUI polkit agent specifically was not driven interactively — verification used `pkttyagent`'s text-mode agent, which authorizes through the identical polkit authority backend. This is a cosmetic/UX gap, not a functional or security one, consistent with the same caveat noted for Phase 2/4 Part 2.

This plan is now considered functionally verified end-to-end. Merge-readiness (branch `phase-5-part2` → `master`) and release/version-bump are coordination decisions for the user, not automated by this verification pass.
