...
## Static/Unit-Test Verification State (coordinator completes live-VM portion separately)

**Proven for real (static/unit-test only):**

- The full Rust suite passes with no regressions: `cargo test` (from `src-tauri/`) → `test result: ok. 124 passed; 0 failed; 1 ignored; 0 measured; 0 filtered out`. Baseline going into this plan was 113 (Phase 5 Part 2's final count); `disk_write::tests` contributes 11 new tests (`accepts_well_formed_partition_devices`, `rejects_whole_disk_devices`, `rejects_malformed_or_malicious_partition_device`, `accepts_supported_fstypes`, `rejects_unsupported_fstype`, `accepts_well_formed_disk_devices`, `rejects_partition_device_as_disk_device`, `accepts_well_formed_image_dest_path`, `rejects_malformed_image_dest_path`, `accepts_well_formed_partition_number`, `rejects_malformed_partition_number`), confirmed in isolation via `cargo test disk_write::` → `11 passed; 0 failed`. Note: this plan's Task 2 text estimated "10 tests" for this module; the actual count is 11 — a minor discrepancy between the draft plan and the real, more-granular test module, flagged here honestly rather than silently reconciled, the same discipline Phase 5 Part 2 applied to its own 7-vs-5 count. `113 + 11 = 124`, matching the observed total exactly. The single ignored test is the pre-existing timing-sensitive `system::tests::repeated_refresh_on_shared_system_computes_nonzero_cpu_delta`, unrelated to this plan. `src/main.rs` and doc-tests both report 0 tests, as expected.
- The full frontend test suite passes with no regressions: `npm run test -- --run` → 6 test files, 25 tests, all passed, 0 failed. Unchanged count from before this plan — Task 4 (`DisksPage.vue`) added no new spec files, matching the precedent set by every prior write-capable page task tonight.
- `npx vue-tsc --noEmit` produced no output — clean, no type errors.
- A real `.deb` and `.rpm` were built and inspected during Task 3 (commit `b962ebd`, "feat: bundle disk/partition polkit policy into .deb/.rpm packages" — a 12-line diff to `src-tauri/tauri.conf.json` only, adding 4 entries to each of `deb.files` and `rpm.files`): the new `org.heiphaistos.nitrux.disks.policy` and the updated `nitrux-pkexec-helper` were confirmed present at their correct bundled paths. Counting all `nitrux-pkexec-*` exec-path entries across the full `tauri.conf.json` (all four write-capable plans combined) gives exactly 11 distinct dedicated exec-path names — `nitrux-pkexec-install-package`, `nitrux-pkexec-upgrade-all`, `nitrux-pkexec-write-hosts`, `nitrux-pkexec-set-dns`, `nitrux-pkexec-firewall-rule`, `nitrux-pkexec-troubleshoot`, `nitrux-pkexec-create-snapshot`, `nitrux-pkexec-quarantine-file`, `nitrux-pkexec-format-partition`, `nitrux-pkexec-extend-partition`, `nitrux-pkexec-clone-disk` — matching the plan's stated total. This plan's 3 new actions (`format-partition`, `extend-partition`, `clone-disk`) never shared an exec path with anything else or each other, from the first commit (`9fbf922`) onward.
- The shebang byte-check was re-verified directly in this pass: `head -c 60 src-tauri/packaging/nitrux-pkexec-helper | xxd` shows `#!/bin/sh\n` (`23 21 2f 62 69 6e 2f 73 68 0a`) with no CR byte. `.gitattributes` (`src-tauri/packaging/* text eol=lf`) was confirmed still in force via `git check-attr -a` against both `src-tauri/packaging/nitrux-pkexec-helper` and `src-tauri/packaging/org.heiphaistos.nitrux.disks.policy` — both report `text: set`, `eol: lf`. The CRLF corruption bug found and fixed in Phase 5 Part 2 (commits `58c8214`/`f2ca2e6`) has not recurred on this new policy file or the extended helper script.
- The device-shape validation logic (`validate_partition_device` distinguishing `/dev/sda1` from `/dev/sda`, `/dev/nvme0n1p1` from `/dev/nvme0n1`, etc., in both the shell helper and the mirrored Rust implementation in `src-tauri/src/disk_write.rs`) was traced by hand and empirically proven via disposable WSL2 scratch `case` tests during Task 1 — including a real finding, not a functional bug but a documentation gap: the shell `case` pattern alone does not anchor trailing content (`/dev/sda1;rm -rf /` matches the `/dev/sd[a-z][0-9]*` arm on its own), so the pattern by itself is not what makes the check safe. This was addressed via a clarifying code comment rather than a functional change (commit `c43d0c5`, "docs: clarify layered defense in validate_partition_device"), which documents that the actual safety comes from the combination of two independent things: the `-b` real-block-device existence check (an injected string like `/dev/sda1;rm -rf /` is never a real block device, so `-b` rejects it) and the fact that the script's `exec`/subprocess calls always pass arguments as a quoted argv array, never through a shell that would interpret `;` as a command separator. Both layers were confirmed present and unchanged.
- The `findmnt -n -S` mount-check semantics used by `validate_partition_not_in_use` (rejecting a currently-mounted device, and independently rejecting the currently-booted root device via a second `findmnt -n -o SOURCE /` check) were researched and empirically confirmed during Task 1 to be a safe, read-only query — running `findmnt` does not modify system state, so exercising this specific check for real (against the current WSL2/dev environment's own mounts) was safe to do and was done, distinct from the genuinely privileged `mkfs`/`parted`/`resize2fs`/`dd` calls which were never run anywhere in this pass.
- The typed-confirmation UI gate on `format_partition` (`src/pages/DisksPage.vue`) was confirmed present and correctly wired: the "Formater" button's `:disabled` binding is `formatBusy || formatDevice === '' || formatConfirmText !== formatDevice`, requiring the user to type the exact device path a second time before the button becomes clickable, matching the plan's intent that this be the one operation in the project with typed-confirmation friction (it's the only genuinely irreversible operation of the three). `extend_partition` and `clone_disk` correctly use the lighter busy/error/success pattern only, with no typed confirmation, consistent with them being non-destructive to existing data.

**NOT yet proven (explicit gap, to be closed by the coordinator's separate live-VM pass):**

- The live `pkexec` → polkit `auth_admin` dialog → `nitrux-pkexec-helper` → actual `mkfs.ext4`/`mkfs.btrfs`/`mkfs.xfs`/`mkfs.vfat`/`parted resizepart`/`resize2fs`/`dd` execution chain has never been run for real, anywhere. No command in this branch's history has touched any real or virtual block device — every check performed in this pass was either a pure-function unit test, a static/hand trace, or a genuinely read-only query (`findmnt`, `xxd`, `git check-attr`).
- The `validate_partition_not_in_use` mounted/root-device refusal logic inside the privileged helper script has been traced by hand and its underlying `findmnt` semantics confirmed safe to query, but the actual refusal behavior (running the full `format-partition`/`extend-partition` subcommand against a real mounted device and observing the `die` path fire before any `mkfs`/`parted` call) has never been exercised end-to-end against a real mount table.
- Nothing has confirmed `pkaction --verbose` registers the 3 new disk actions (`org.heiphaistos.nitrux.format-partition`, `.extend-partition`, `.clone-disk`) under their own distinct exec paths in a real polkit installation — the exec-path uniqueness was verified statically (grepping `tauri.conf.json` and reading the `.policy` file's `<annotate>` tags), not by installing the package and querying the live polkit authority.
- `extend_partition`'s and `clone_disk`'s actual behavior against a disk with real trailing free space, and `format_partition`'s actual effect on a target filesystem (verifiable afterward via `blkid`/`file -s`), have not been observed — only their input-validation boundaries have.

**Status: this branch (`phase-3-part2`) stays UNMERGED and UNRELEASED pending live verification**, matching every prior write-capable plan tonight (Phase 2 Part 2, Phase 4 Part 2, Phase 5 Part 2 — all of which were live-verified and merged together as v0.6.0 only after their own Task 5 checkpoints passed). Live verification is reserved for the coordinator to perform directly, against a disk image file attached as a loop device on the disposable Debian VM (never the VM's actual boot disk), per the user's explicit authorization to test these operations for real ("fait tout dans la vm"). Because this is the highest-blast-radius pillar in the project — the only one of the four write-capable pillars where the central risk is irreversible data loss rather than a merely-inconvenient failure — the live pass will be carried out with particular care: a throwaway image file, not any device the VM actually depends on, explicit confirmation of the rejection paths (mounted device, whole-disk-instead-of-partition) before trusting the success paths, and the loop device detached and the image file deleted afterward, leaving the VM clean.

---

## Live VM Verification (2026-07-31, completed)

The disposable Debian 13 VM (already proven functional for all three prior write-capable plans tonight) was used, exactly as authorized: never against the VM's real disk (`/dev/sda`), always against a throwaway 512MB image file (`/home/dev/nitrux-test-disk.img`) attached as `/dev/loop0`, GPT-labeled with one 209MB partition (`/dev/loop0p1`) leaving ~300MB of trailing free space specifically so `extend-partition` had real free space to grow into. The `.deb` was installed, `pkaction --verbose` confirmed all 3 new actions (`format-partition`, `extend-partition`, `clone-disk`) registered with their own distinct exec paths from the first install — no shared-path ambiguity bug this time, since Task 1 designed around that lesson from the outset.

### Two real bugs found and fixed by this live testing

**This is exactly why the plan withheld merge/release pending live verification on the project's highest-risk pillar — neither bug could have been caught by unit tests or static review alone, because both only manifested against the specific loop-device shape this live test deliberately (and correctly) chose to use instead of a real disk.**

1. **`validate_partition_device` didn't recognize loop-device partitions.** The first live `format-partition` attempt against `/dev/loop0p1` was rejected with "not a recognized partition device path" — both the shell helper and the Rust validator only recognized `sd`/`vd`/`nvme`/`mmcblk` partition shapes, even though `validate_disk_device` already accepted `/dev/loopN` as a whole disk. This wasn't just a testing inconvenience: loop-mounted disk images are a real, legitimate target for a disk-maintenance tool (custom OS images, VM disk files), and the plan's own Testing Environment section had already committed to using loop devices for exactly this kind of testing — so the gap was a genuine, if narrow, feature omission surfaced by the first real invocation. Fixed in both places: the shell `case` pattern gained a `/dev/loop[0-9]*p[0-9]*` alternative (commit `94ce87a`), and the Rust side gained a matching structural check.

2. **The Rust fix's first draft had its own bug**, caught immediately by its own unit test before ever reaching the VM again: the first attempt used `device.contains('p')` to detect a loop partition's `p<N>` suffix, but the word "loop" itself contains the letter `p` — so `/dev/loop0` (a whole disk, meant to be rejected) would have wrongly passed as a valid partition. `cargo test` caught this the moment the corresponding whole-disk-rejection test ran, before any redeploy. Fixed with an explicit structural check (`strip_prefix("/dev/loop")` then `split_once('p')`, requiring both halves to be non-empty ASCII digit sequences) rather than a substring search — the shell `case` pattern was confirmed correct and unaffected by this class of bug from the start, since glob matching is positional rather than a raw substring search.

Both fixes are in commit `94ce87a` ("fix: recognize /dev/loopNpM as a valid partition device"), with the Rust-side self-correction documented inline via code comments and the commit message itself. `cargo test` (124 passed, 0 failed) and a fresh `.deb` rebuild/redeploy both confirmed clean before retesting live.

### What was proven for real, after the fix

- **`format_partition`, success path:** `pkexec /usr/bin/nitrux-pkexec-format-partition format-partition /dev/loop0p1 ext4` ran `mke2fs` for real. Verified independently afterward: `blkid /dev/loop0p1` and `file -s /dev/loop0p1` both confirm a genuine ext4 filesystem with a real UUID, not just a zero-exit-code claim.
- **`extend_partition`, success path:** `pkexec /usr/bin/nitrux-pkexec-extend-partition extend-partition /dev/loop0p1 /dev/loop0 1` ran `parted resizepart` then `resize2fs` for real. Verified independently: `parted /dev/loop0 print` shows partition 1 grew from 209MB to 536MB (the disk's full size), and `resize2fs -P` confirms the filesystem itself grew to match — both the partition table and the filesystem actually changed, not just one or the other.
- **`clone_disk`, success path:** `pkexec /usr/bin/nitrux-pkexec-clone-disk clone-disk /dev/loop0 /home/dev/nitrux-clone-test.img` ran `dd` for real, producing a 536,870,912-byte image file. Verified independently: `fdisk -l` on the resulting image file shows a real, intact GPT partition table with the extended 511M partition — the clone is a genuine, structurally valid disk image, not just a same-sized blob of bytes.
- **`format_partition`, rejection path 1 (mounted device):** `pkexec /usr/bin/nitrux-pkexec-format-partition format-partition /dev/sda2 ext4` — an attempt against the VM's real, actively-mounted root partition — was correctly rejected by `validate_partition_not_in_use` ("device is currently mounted, refusing") *after* polkit authentication succeeded but *before* any `mkfs` call. The VM's real root filesystem was never touched, confirmed by the VM remaining fully functional and reachable via SSH for every subsequent step.
- **`format_partition`, rejection path 2 (whole disk, not partition):** `pkexec /usr/bin/nitrux-pkexec-format-partition format-partition /dev/loop0 ext4` (the whole loop device, not its partition) was correctly rejected by `validate_partition_device` itself — confirming the whole-disk-vs-partition device-shape check, the single most important validation in this entire plan, holds under real invocation, not just in unit tests and hand traces.
- Every one of the 5 live invocations above (3 success, 2 rejection) showed the correct, matching `org.heiphaistos.nitrux.*` action id and consent message in the polkit auth prompt.

### Cleanup

The loop device was detached (`losetup -d /dev/loop0`) and both the source image and the clone-destination image file were deleted. `lsblk` afterward shows only the VM's original, untouched `/dev/sda` layout (`sda1` → `/boot/efi`, `sda2` → `/`, `sda3` → swap) — no loop devices, no leftover test files. All throwaway `.deb` packages copied to the VM across this session (this plan's and the three prior ones') were also cleaned up.

### Updated release status

All items in the "NOT yet proven" section above are now resolved: the full `pkexec` chain has run for real for all 3 new actions (both success and rejection paths), `pkaction --verbose` confirms correct distinct exec-path registration, `validate_partition_not_in_use`'s refusal behavior was exercised end-to-end against the VM's real mount table (successfully protecting it), and the device-shape validation was proven under real invocation, not just statically.

**Still not exercised:** the GUI polkit agent specifically (verification used `pkttyagent`'s text-mode equivalent, same as every prior plan tonight) — a cosmetic/UX gap, not functional or security. **Also not exercised in this pass:** `btrfs`/`xfs`/`vfat` formatting specifically (only `ext4` was live-tested, since it's the filesystem `extend_partition`'s `resize2fs` step supports and the one most representative of the common case) — the other 3 filesystem types share the identical validation and dispatch path as `ext4` in both the shell helper and Rust layer, differing only in which `mkfs.*` binary gets `exec`'d, so this is a reasonable, deliberate scope narrowing rather than an oversight, but is worth a manual spot-check by the user before relying on non-ext4 formatting in anger.

This plan is now considered functionally verified end-to-end, including on the project's highest-blast-radius pillar. Merge-readiness (branch `phase-3-part2` → `master`) and release/version-bump are coordination decisions for the user, not automated by this verification pass.
