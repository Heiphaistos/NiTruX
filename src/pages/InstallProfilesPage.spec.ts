...
describe("InstallProfilesPage", () => {
  it("lists every profile with its app count", () => {
    ...
    expect(wrapper.text()).toContain("Ligne de commande modernisée");
  });
  ...
  it("selecting the new 'Cloud & Synchronisation' profile checks all its real appCatalog entries", async () => {
    const wrapper = mount(InstallProfilesPage);
    const button = wrapper.findAll("button").find((b) => b.text().includes("Cloud"))!;
    await button.trigger("click");
    const checkboxes = wrapper.findAll("input[type=checkbox]:checked");
    expect(checkboxes.length).toBe(5);
  });

  it("selecting the new 'Productivité' profile checks all its real appCatalog entries", async () => {
    const wrapper = mount(InstallProfilesPage);
    const button = wrapper.findAll("button").find((b) => b.text().includes("Productivité"))!;
    await button.trigger("click");
    const checkboxes = wrapper.findAll("input[type=checkbox]:checked");
    expect(checkboxes.length).toBe(4);
  });

  it("selecting the new 'Émulation & compatibilité' profile checks all its real appCatalog entries", async () => {
    const wrapper = mount(InstallProfilesPage);
    const button = wrapper.findAll("button").find((b) => b.text().includes("Émulation"))!;
    await button.trigger("click");
    const checkboxes = wrapper.findAll("input[type=checkbox]:checked");
    expect(checkboxes.length).toBe(5);
  });
  ...
  it("selecting the new 'Ligne de commande modernisée' profile checks all its real appCatalog entries", async () => {
    const wrapper = mount(InstallProfilesPage);
    const button = wrapper.findAll("button").find((b) => b.text().includes("Ligne de commande modernisée"))!;
    await button.trigger("click");
    const checkboxes = wrapper.findAll("input[type=checkbox]:checked");
    expect(checkboxes.length).toBe(5);
  });
